<template>
  <div class="my-3 mx-5 text-white">
    <el-scrollbar height="600px">
      ...
      <div class="border border-solid rounded-sm border-gray-700 text-center cursor-pointer h-10 flex flex-col justify-center"
           @click="save"><span>快速保存</span></div>
    </el-scrollbar>
    <right-file-select-dialog ref="rightFileSelectDialog"
                              @save="updateForm"
                              type="radio"></right-file-select-dialog>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    save () {
      this.http.post('http://192.168.10.10/api/v2/resource', this.form).then((res) => {
        console.log(res)
      });
    },
